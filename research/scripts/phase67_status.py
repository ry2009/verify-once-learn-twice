...
SPEC = "data/ablation_phase67_t1l2r1_8b_t50_s012.json"
...
def _best_run_dir_for_name(group_dir: Path, run_name: str) -> Path | None:
    ...
    for run_dir in group_dir.iterdir():
        ...
        complete, _, _ = _status_for_run_dir(run_dir)
        cands.append((1 if complete else 0, stamp, run_dir))
    if not cands:
        return None
    # Prefer completed runs; if none completed, use latest live run.
...
def main() -> None:
        ...
        if not group_dir.exists():
            views.append(
                RunView(
                    ...
                    False,
                    0,
                    int(spec.get("max_steps", 0)),
                    "missing",
                )
            )
            all_complete = False
            continue
        latest = _best_run_dir_for_name(group_dir, run_name)
        if latest is None:
            views.append(
                RunView(
                    run_name,
    ...
    for v in views:
        print(
            f"  - [{v.state}] method={v.method:<16} task_done={v.task_done:>3}/{v.target:>3} run={v.run_name}"
        )
    print()
